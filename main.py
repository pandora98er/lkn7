...
confession_count = 131  # Định nghĩa biến confession_count
confession_threads = {}
    ...
    if confession_channel and log_channel and reply_log_channel:
        ...
        async for log_message in log_channel.history(limit=200):
            if f"[Confession #{confession_number}]" in log_message.content:
                ...
                parts = log_message.content.split('(Message ID: ')[1].split(
                    ', Thread ID: ')
                ...
                if thread:
                    reply_embed = discord.Embed(
                        description=
                        f"> <a:Zdomdomlkn:1235512517359308840> {reply_text}")

                    # Lấy thông tin người đăng bài từ tin nhắn log
                    original_author_id = int(
                        log_message.content.split(')')[0].split('(')[1])
                    original_author = await bot.fetch_user(
                        original_author_id)

                    # Kiểm tra xem người dùng là người đăng bài gốc hay không
                    if ctx.author.id == original_author.id:
                        reply_embed.set_footer(
                            text="Phản hồi bởi người đăng bài")
                    else:
                        reply_embed.set_footer(
                            text="Phản hồi bởi người dân trong làng")
        ...
        await ctx.send(
            f"Không tìm thấy confession #{confession_number} trong dữ liệu")
    else:
        await ctx.send(
            "Không tìm thấy kênh confession, log hoặc kênh phản hồi, vui lòng cài đặt lại đúng cách."
        )
